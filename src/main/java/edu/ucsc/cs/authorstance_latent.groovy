...
def testdir = 'data'+java.io.File.separator + subDir + java.io.File.separator + fold + java.io.File.separator + 'train' + java.io.File.separator;
...
model.add rule : (agrees(A1, A2, T) & (A1-A2) & participates(A2, T) & isProAuth(A1, T)) >> isProAuth(A2, T), weight : initialWeight
model.add rule : (agrees(A1, A2, T) & (A1-A2) & participates(A1, T) & participates(A2, T) & ~(isProAuth(A1, T))) >> ~(isProAuth(A2, T)), weight :initialWeight

model.add rule : (sarcastic(A1, A2, T) & (A1-A2) & participates(A2, T) & isProAuth(A1, T)) >> ~isProAuth(A2, T), weight : initialWeight
model.add rule : (sarcastic(A1, A2, T) & (A1-A2) & participates(A1, T) & participates(A2, T) & ~(isProAuth(A1, T))) >> (isProAuth(A2, T)), weight :initialWeight

model.add rule : (nasty(A1, A2, T) & (A1-A2) & participates(A2, T) & isProAuth(A1, T)) >> ~isProAuth(A2, T), weight : initialWeight
model.add rule : (nasty(A1, A2, T) & (A1-A2) & participates(A1, T) & participates(A2, T) & ~(isProAuth(A1, T))) >> (isProAuth(A2, T)), weight :initialWeight

model.add rule : (attacks(A1, A2, T) & (A1-A2) & participates(A2, T) & isProAuth(A1, T)) >> ~isProAuth(A2, T), weight : initialWeight
model.add rule : (attacks(A1, A2, T) & (A1-A2) & participates(A1, T) & participates(A2, T) & ~(isProAuth(A1, T))) >> (isProAuth(A2, T)), weight :initialWeight
...
Evaluator evaluator = new Evaluator(testDB, isProAuth, "psl_authorstance_latent", fold);
...
/*output prediction results */
evaluator = new Evaluator(testDB, hasIdeologyA, "ideologyA_authorstance", fold);
evaluator.outputToFile();
...
try {
    ...
    ResultWriter rs = new ResultWriter(score, fold, 'result_latent.txt')
    rs.write()
