...
model.add rule : (agreesAuth(A1, A2, T) & (A1-A2) & hasIdeologyA(A1)) >> hasIdeologyA(A2), weight : initialWeight
model.add rule : (agreesAuth(A1, A2, T) & (A1-A2) & ~hasIdeologyA(A1)) >> ~hasIdeologyA(A2), weight : initialWeight

model.add rule : (disagreesAuth(A1, A2, T) & (A1-A2) & ~hasIdeologyA(A1)) >> hasIdeologyA(A2), weight : initialWeight
model.add rule : (disagreesAuth(A1, A2, T) & (A1-A2) & hasIdeologyA(A1)) >> ~hasIdeologyA(A2), weight : initialWeight
...
Database distributionDB = data.getDatabase(predict_tr, [hasLabelPro, agreesAuth, disagreesAuth, hasTopic, writesPost, topic] as Set, observed_tr);
...
Database testDB = data.getDatabase(predict_te, [hasLabelPro, agreesAuth, disagreesAuth, participates, hasTopic, writesPost, topic] as Set, observed_te);
...
/*output prediction results */
evaluator = new Evaluator(testDB, hasIdeologyA, "ideologyA_authors_agrees", fold);
evaluator.outputToFile();
